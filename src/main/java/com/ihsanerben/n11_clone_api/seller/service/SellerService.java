...
public class SellerService {
	...
	public SellerResponse getMyApplication() {
		Long userId = authenticatedUser.userId();
		return sellers.findByUserId(userId).map(mapper::toResponse).orElseThrow(SellerNotFoundException::new);
	}

	@Transactional(readOnly = true)
	public Page<SellerResponse> list(SellerStatus status, Pageable pageable) {
		Page<Seller> result = status == null ? sellers.findAll(pageable) : sellers.findAllByStatus(status, pageable);
