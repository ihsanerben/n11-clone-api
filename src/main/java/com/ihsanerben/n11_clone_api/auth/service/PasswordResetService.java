...
public class PasswordResetService {
	...
	public void resetPassword(ResetPasswordRequest request) {
		PasswordResetToken token = resetTokens.findByTokenHash(tokenService.hash(request.token()))
	...
	private void issueResetToken(User user) {
		...
		resetTokens.save(PasswordResetToken.builder()
				...
				.expiresAt(Instant.now().plus(Duration.ofMinutes(properties.passwordResetExpirationMinutes())))
				.used(false)
