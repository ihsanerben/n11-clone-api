...
class SellerControllerIT {
	...
	void shouldApplyApproveAndEnforceRoleBoundaries() throws Exception {
		...
		mockMvc.perform(put("/api/admin/sellers/{sellerId}/approve", sellerId)
				.header("Authorization", "Bearer " + adminToken))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.status").value("APPROVED"));

		assertThat(users.findById(applicant.getId()).orElseThrow().getRole()).isEqualTo(UserRole.SELLER);
	}
	...
	void shouldResubmitRejectedApplicationUsingSameSellerRow() throws Exception {
		...
		mockMvc.perform(post("/api/sellers/apply").header("Authorization", "Bearer " + userToken)
				.contentType(APPLICATION_JSON).content("{\"storeName\":\"First Store\"}"))
				.andExpect(status().isCreated());
		Long originalId = sellers.findByUserId(applicant.getId()).orElseThrow().getId();
		mockMvc.perform(put("/api/admin/sellers/{sellerId}/reject", originalId)
				.header("Authorization", "Bearer " + adminToken))
				.andExpect(status().isOk());
		mockMvc.perform(post("/api/sellers/apply").header("Authorization", "Bearer " + userToken)
				.contentType(APPLICATION_JSON).content("{\"storeName\":\"Second Store\"}"))
				.andExpect(status().isCreated())
	...
	private User saveUser(String username, UserRole role) {
		return users.saveAndFlush(User.builder().username(username).email(username + "@example.com")
				...
				.role(role).emailVerified(true).createdAt(Instant.now()).build());
	}
}
