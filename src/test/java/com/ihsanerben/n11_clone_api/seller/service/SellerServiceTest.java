...
class SellerServiceTest {
	...
	void shouldApprovePendingSellerAndPromoteUserInOneOperation() {
		User user = User.builder().id(7L).username("seller").email("seller@example.com")
				.role(UserRole.USER).build();
		Seller seller = Seller.builder().id(11L).user(user).storeName("Store")
				.status(SellerStatus.PENDING).appliedAt(Instant.now()).build();
		when(sellers.findById(11L)).thenReturn(Optional.of(seller));
		SellerService service = new SellerService(sellers, users, new SellerMapper(), authenticatedUser);
